...
import Osm from "../../components/Maps/OpenStreetMap.vue";
export default {
  components: {
    "open-street-map": Osm,
  },
  created() {
    EventBus.$on("addMarker", this.save);

  },
  destroyed() {
    EventBus.$off("addMarker", this.save);
  ...
  methods: {
    ...
    save() {
        let address = this.formData.title + ', '+this.wardTitle +', '+this.districtTitle +', '+this.provinceTitle;
        let marker = {
            lat : this.formData.lat,
            lng : this.formData.lng
        };
        let location = {address : address,marker : marker};
        this.$emit('addToListAddress',location)
    },
  },
  mounted() {},
};
