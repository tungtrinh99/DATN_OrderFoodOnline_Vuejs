  ...
  methods: {
   getOverviewData(){
     ...
     .then((res)=>{
       ...
       this.overviewCard[0].count = res.data.data.items.reduce(function(accumulator, currentValue){
         return accumulator + currentValue.grand_total;
       },0);
       this.overviewCard[1].count = res.data.data.items.reduce(function(accumulator, currentValue){
...
<style scoped>
.overview-card, .agency-chart, .sale-chart {
  padding: 20px;
}
.overview-title, .chart-title {
  margin-bottom: 24px;
}
.overview-title > h4, .chart-title > h4 {
  font-weight: bold;
  font-size: 20px;
  color: #252422;
}
